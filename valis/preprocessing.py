...
def mask2contours(mask, kernel_size=3):
    # print(mask)
    assert mask.dtype == np.uint8, "The mask should be of type uint8!"
    assert np.max(mask) <= 255 and np.min(mask) >= 0, "Invalid mask values!"
    # if not np.any(mask):
    #     raise ValueError("The input mask is empty!")
    assert mask.any(), "[Modified] The input mask is empty!"
    ...
    if not np.any(contour_mask):
        raise ValueError("Failed to draw contours on the mask!")
