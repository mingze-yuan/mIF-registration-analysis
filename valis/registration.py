...
class Valis(object):
    ...
    def __init__(self, src_dir, dst_dir, series=None, name=None, img_type=None,
        ...
        ###### change
        def replace_file(fname_pth: str):
            if not fname_pth.endswith('.vsi'):
                return fname_pth
            fname_dir = os.path.dirname(fname_pth)
            fname = os.path.basename(fname_pth).removesuffix('.vsi')
            return os.path.join(fname_dir, f"_{fname}_", 'stack1', 'frame_t.ets')
        replaced_list = list(map(replace_file, self.original_img_list))
        self.original_img_list = replaced_list
        ###### change

        print(self.original_img_list)

        if self.name_dict is None:
            if self.original_img_list[0].endswith('.ets'):
                self.name_dict = self.get_img_names_specific(self.original_img_list)
            else:
                self.name_dict = self.get_img_names(self.original_img_list)

        print(self.name_dict, self.original_img_list)
    ...
    def get_img_names_specific(self, img_list):
        """
        img_list be like ['/home/yuanmingze/data/mIF_P1_demo/_Image_222476-P1-01_/stack1/frame_t.ets']
        """
        def get_name_specific(img_pth: str):
            """
            img_pth be like '/home/yuanmingze/data/mIF_P1_demo/_Image_222476-P1-01_/stack1/frame_t.ets'
            """
            patient_dir_name = img_pth.split('/')[-3]
            img_name = patient_dir_name.split('-', 1)[1].removesuffix('_')
            return img_name

        names_dict = {f: get_name_specific(f) for f in img_list}
        return names_dict
    ...
    def get_all_overlap_mask(self, rigid_registrar):
        ...
        assert combo_mask.any(), '[Modified] The mask before hysteresis threshold is already empty!'
        print(combo_mask.min(), combo_mask.max(), self.size)
        temp_mask = 255*filters.apply_hysteresis_threshold(combo_mask, 0.5, self.size-0.5).astype(np.uint8)
        if not temp_mask.any():
            print(f'[Modified] Initial threshold for hysteresis failed, try manual threshold.')
            # min_val = combo_mask.min()
            max_val = combo_mask.max()
            ...
            # # Compute thresholds
            # low_threshold = min_val + low_fraction * data_range
            # high_threshold = min_val + high_fraction * data_range
            low_threshold, high_threshold = 0.5, max_val - 0.5
            temp_mask = 255*filters.apply_hysteresis_threshold(combo_mask, low_threshold, high_threshold).astype(np.uint8)

        assert temp_mask.any(), '[Modified] The mask after hysteresis threshold is empty!'
        mask = 255*ndimage.binary_fill_holes(temp_mask).astype(np.uint8)
        
        assert mask.any(), "[Modified] The mask is empty!"
        # print(mask, mask.shape, mask.sum())
        mask = preprocessing.mask2contours(mask)
        assert mask.any(), "[Modified] The mask is empty after mask2contours!"
    ...
    def _create_non_rigid_reg_mask_from_rigid_masks(self):
        ...
        if not temp_non_rigid_mask.any():
            print(f'[Modified] Initial threshold for hysteresis failed, try manual threshold.')
            # min_val = combo_mask.min()
            max_val = combo_mask.max()
            # data_range = max_val - min_val
            ...
            # high_threshold = min_val + high_fraction * data_range
            low_threshold, high_threshold = 0.5, max_val - 0.5
            temp_non_rigid_mask = 255*filters.apply_hysteresis_threshold(combo_mask, low_threshold, high_threshold).astype(np.uint8)

        assert temp_non_rigid_mask.any(), '[Modified] The mask after hysteresis threshold is empty!'
        # mask = 255*ndimage.binary_fill_holes(temp_mask).astype(np.uint8)
        ...
        assert non_rigid_mask.any(), '[Modified] The mask after binary filling is empty!'
    ...
    def register_micro(self,  brightfield_processing_cls=DEFAULT_BRIGHTFIELD_CLASS,
        ...
        try:
            ...
            print(f"[Modified] Fail to save micro registration overlap!")
